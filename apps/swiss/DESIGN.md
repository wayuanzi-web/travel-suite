...
## 15. 1.6 — 伴手禮升級為主分頁＋單品深度頁（2026-08-03）

**問題**：購物埋在「更多→購物→清單」三層下，且點開只看到 TOP3；選定某品項後「不要買哪款、網友怎麼說、去哪買」散落在店家頁與怎麼選頁，現場對不起來。

**改動**
1. **🎁購物升為底部主分頁**（共 8 頁），「更多」內的重複入口移除。
2. **🛒 我的清單**（新，預設頁）：任一品項按「🛒想買」加入；清單顯示待買／已買計數、離境倒數，逾期品項標紅；一鍵「✅買到了」。存 `wang.swiss.buy`。
3. **📖 完整買法（單品全螢幕詳情）** — `giftDossier()` 把散在四份資料裡的東西按品項聚合成一頁：
   - ✅ 買哪一款（TOP3：款式／價格／適合送誰）
   - 🚫 **不要買／避雷**（自店家 `warns` 與城市 `skip` 交叉比對，如 Läderach 班霍夫大道週末公休、策馬特 Sprüngli 產品線不全、聖莫里茲某店歐元匯率坑客）
   - 💡 怎麼挑（品牌 `howto` 關鍵知識，如認 Swiss made、超市便宜 30–40%）
   - 💬 網友怎麼說（該品牌各店 `intel` 匯總）
   - 🏬 去哪買（含送長官／同事／自家人分層建議、一鍵導航、電話、複製給店員的英文）
   - 🛒 現場怎麼買（步驟＋口味）｜⭐ 深度比價｜同類選購指南
4. 採購倒數列可直接點進單品詳情。

**交叉索引方法**：以 gift `id` 為軸，比對 `itemMap`、店家 `brand`/`name`、`skip` 文字，聚合 `brands`／`top3`／`buyGuide`（前綴比對）／`fieldGuide`／`flavor`（依分類）／`deadlines`。
