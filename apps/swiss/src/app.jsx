  ...
  return (
    <div style={S.app}>
      ...
      <nav style={{ ...S.nav, paddingBottom: "env(safe-area-inset-bottom)" }}>
        {[["today","🗓️","行程"],["nav","🧭","導航"],["eat","🍽️","餐廳"],["tickets","🎫","票券"],["wx","🌡","天氣"],["phrases","🗣️","會話"],["more","☰","更多"]].map(([k,ic,lb]) => (
          <button key={k} onClick={()=>setTab(k)} style={{...S.navBtn, ...(tab===k?S.navActive:{})}}>
            <div style={{fontSize:20}}>{ic}</div><div style={{fontSize:11}}>{lb}</div>
          </button>
        ))}
      </nav>
    </div>
  ...
  return (
    <div>
      <div style={{display:"flex", gap:6, marginBottom:10, overflowX:"auto", paddingBottom:2}}>
        {[["shop","🛍購物"],["depart","🛫離境"],["docs","📁證件"],["tools","🧰工具"],["sos","🛡️應變"],["lost","🆘走散"],["help","📱說明"]].map(([k,lb])=>(
